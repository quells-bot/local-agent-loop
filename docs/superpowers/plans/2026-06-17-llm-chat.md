...
> This references `session::ChatSession`, `activities::{build_request, parse_response, ChatRequest, LlmComplete, OaiMessage}` which land in Tasks 3–4. Until those exist the crate will not compile, so create the `session.rs` stub now to keep the build green for this task:
...
And temporarily trim `lib.rs` to only what exists after this task — replace the two `pub use` lines above with:
...
(Task 3 re-adds the `build_request`/`parse_response`/`ChatRequest`/`LlmComplete`/`OaiMessage` exports; Task 4 re-adds `ChatSession`.)
...
**Files:**
- Modify: `crates/chat/src/session.rs` (replace the stub)
- Modify: `crates/chat/src/lib.rs` (re-add the `ChatSession` re-export)
...
Replace the contents of `crates/chat/src/session.rs`:
...
- [ ] **Step 2: Re-add the `ChatSession` re-export**

In `crates/chat/src/lib.rs`, add (or restore) the line:
